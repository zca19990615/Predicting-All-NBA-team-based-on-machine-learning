...
# 设置交叉验证，考虑类别不平衡处理，使用SMOTE进行平衡处理
train_control <- trainControl(method = "repeatedcv", 
                              number = 10, 
                              repeats = 3,
                              sampling = "smote")  # 使用SMOTE平衡类别

# 训练KNN模型，并使用交叉验证
knn_model <- train(Injured ~ ., data = trainSet, method = "knn", trControl = train_control)

# 在训练集上进行预测
train_predictions <- predict(knn_model, trainSet)
...
bootstrap_train_metrics <- function(data, indices) {
  ...
  # 进行预测
  predictions <- predict(knn_model, boot_sample)
...
# 在测试集上进行预测
test_predictions <- predict(knn_model, testSet)
...
ggplot(plot_data, aes(x = Metric, y = Mean, color = Set)) +
  ...
  labs(title = "KNN Model Performance with 95% Confidence Intervals",
